-- Decline path for interleague invites: full-invite decline + per-game decline.
-- The existing status check on interleague_invites already allows 'declined'.

alter table public.interleague_invites
  add column decline_reason text;

-- ── accept_interleague_invite: per-game `action='decline'` now deletes the game
...
-- ── decline_interleague_invite: full-invite decline ───────────────────────────
