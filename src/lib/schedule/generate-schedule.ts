...
// Placeable-slot grid — extracted verbatim into a directive-free module so a
// server component can share it (see slots.ts header). `timeToMinutes` and
// `weekKey` are the only helpers the rest of this file still calls; the others
// (pad2/minutesToTimeStr/localDateStr/DAY_TO_JS/JS_TO_DAY) live solely inside
// the moved code. buildSlots/buildPlayingDates are re-exported below to keep
// their existing importers (scarcity-order, the sims) unchanged.
import {
  buildSlots,
  buildPlayingDates,
  timeToMinutes,
...
// ─── Internal types ────────────────────────────────────────────────────────────
// DivisionSettings and Slot now live in ./slots (imported above) alongside the
// buildSlots/buildPlayingDates code that defines them.
...
// ─── Pure helpers ──────────────────────────────────────────────────────────────
// timeToMinutes / weekKey (and the slot-grid-only helpers) moved to ./slots and
// are imported above. fmtTime12 stays here — it's presentation, not slot math.
...
// buildPlayingDates and buildSlots moved verbatim to ./slots (imported and
// re-exported at the top of this file). See that module's header for why the
// slot-grid code was lifted out of this "use client" module.
