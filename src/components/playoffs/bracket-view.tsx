...
function GameCard({ game }: { game: GameWithTeams }) {
  ...
  return (
    <div className="w-48 overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
      <TeamRow
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`text-sm font-medium ${
            tbd ? "italic text-gray-400" : "text-gray-900"
          }`}
        >
          {tbd
            ? "TBD vs TBD"
...
export function BracketView({ playoffId, divisionName }: Props) {
  ...
  // Group by round for bracket view
  ...
  const rounds = [...roundMap.entries()].sort(
    ...
    <div className="flex flex-col gap-3">
      ...
      {view === "list" ? (
        <div className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
          {games.map((g, i) => (
            <ListGameRow key={g.id} game={g} idx={i} />
          ))}
        </div>
      ) : (
        /* Bracket tree — horizontal scroll, rounds as columns */
