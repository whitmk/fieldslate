...
function defaultRoleLabel(index: number, total: number, sport?: string | null): string {
  if (total <= 1) return getOfficialTitle(sport);
  // Baseball/softball use the diamond positions; other sports get generic labels.
  if (isUmpireSport(sport)) {
    if (index === 0) return "Plate";
    if (total === 2) return "Field";
    return `Base ${index}`;
  }
  return `${getOfficialTitle(sport)} ${index + 1}`;
}

function syncRoles(prevRoles: string[], nextCount: number, sport?: string | null): string[] {
  const roles: string[] = [];
  for (let i = 0; i < nextCount; i++) {
    roles.push(prevRoles[i] ?? defaultRoleLabel(i, nextCount, sport));
  }
  return roles;
}

export function StepUmpires({ data, update, sport }: Props) {
  const count = data.umpires_per_game;
  const roles = data.umpire_roles;

  const officialTitle = getOfficialTitle(sport);
  const officialTitleLower = officialTitle.toLowerCase();
  const officialPlural = getOfficialTitlePlural(sport);
  const officialPluralLower = officialPlural.toLowerCase();
  ...
  function handleCountChange(nextCount: number) {
    ...
    update({
      ...
      umpire_roles: syncRoles(roles, clamped, sport),
    });
  }
    ...
    <div className="flex flex-col gap-6">
      <div>
        <h3 className="text-lg font-semibold text-[#0C1F3F]">
          {officialTitle} requirements
        </h3>
        <p className="mt-0.5 text-sm text-gray-500">
          Set how many {officialPluralLower} this division needs per game, and what role each one plays.
        </p>
      </div>

      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-medium text-gray-700">
          {officialPlural} per game
        </label>
        <input
          type="number"
          min={0}
        ...
        <p className="text-xs text-gray-400">
          0 means no {officialPluralLower} are required for this division. Max {MAX_UMPIRES}.
        </p>
      </div>
      ...
      {count > 0 ? (
        <div className="flex flex-col gap-3 rounded-xl border border-gray-100 bg-white p-4">
          ...
          <p className="text-xs text-gray-500">
            Label each {officialTitleLower} position. These labels appear on the schedule when {officialPluralLower} are assigned.
          </p>
            ...
            {roles.map((role, idx) => (
              <div key={idx} className="flex items-center gap-3">
                ...
                <input
                  ...
                  placeholder={defaultRoleLabel(idx, count, sport)}
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
          ...
          <p className="text-sm text-gray-600">
            No {officialPluralLower} required. You can come back later and bump this up if officials are needed.
          </p>
        </div>
      )}
    </div>
